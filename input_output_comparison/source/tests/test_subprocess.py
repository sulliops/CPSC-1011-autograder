...
class TestDiff(unittest.TestCase):
    ...
    # Associated point value within Gradescope
    @weight(10)
    def test_InputLowercaseA(self):
        # Title used by Gradescope 
        """Check that input "a" results in correct output"""

        # Create a subprocess to run the students code to obtain an output
        cat = subprocess.Popen(["cat", "input/example.txt"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        test = subprocess.Popen(["./lab2.out"], stdin=cat.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
